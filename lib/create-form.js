...
function isNumberInput(element) {
  return element.getAttribute && element.getAttribute('type') === 'number';
}
...
export const createForm = (config) => {
  ...
  function isFieldPresentInSchema(field) {
    return (
      validationSchema &&
      validationSchema.fields &&
      validationSchema.fields[field]
    );
  }
  function validateFieldValue(field, value) {
    updateTouched(field, true);
    if (isFieldPresentInSchema(field) === undefined) return Promise.resolve();
    if (validationSchema) {
